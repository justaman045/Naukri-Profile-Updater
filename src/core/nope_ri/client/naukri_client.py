...
class NaukriLoginClient:
    ...
    def login(self):
        ...
        self.naukri_session = NaukriSession(token, cookies_to_dict(self.session))
    ...
    @with_exponential_retry(label="fetch_full_profile")
    def _fetch_full_profile(self):
        headers = self._build_headers(auth=True)
        headers["accept"] = "application/json, text/plain, */*"
        return self.session.get(
            FULL_PROFILE_URL,
            headers=headers,
            params={"expand_level": 3},
        )

    @with_exponential_retry(label="download_resume")
    def _download_resume_request(self, url):
        headers = self._build_headers(auth=True)
    ...
    def download_resume(self):
        """Download the current on-file resume PDF as raw bytes."""
        if not self.naukri_session:
            raise NaukriAuthError("Login first")
